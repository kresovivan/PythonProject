...
print(
    my_dict
)  # Изменились значения в исходном словаре, чтобы такого не было нужно использовать#
...
def custom_function4(
    *args, **kwargs
):  # *args, **kwargs можно заменить на *a и **b например
    if args:
        print(args)
        if kwargs:
            print(kwargs)
...
def filter_and_transform(data):
    for item in data:
        if item["age"] > 10:
            yield {"name": item["name"].upper(), "age": item["age"]}
