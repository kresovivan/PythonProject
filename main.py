...
if __name__ == "__main__":
    ...
    print(str1)
#Импорт является довольно затратной операцией, из-за чего Python производит его только один раз на файл
#модуля (однократно на процесс).При последующих операциях импортирования происходит извлечение имен (объекта) из уже
#загруженного ранее модуля. Import и from выступают в роли явного присваивания.
# Импорт связывает наименование модуля с объектом целого модуля
# from связывает одно или несколько имен с объектами с такими же именами из импортируемого модуля.



#Оператор import необходимо всегда применять в случае, когда в различных
#импортируемых модулях на верхнем уровне используется одно и то же имя
